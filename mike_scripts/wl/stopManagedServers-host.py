######################################################
# NAME: stopManagedServers-host.py
#
# DESC: Stops WLS local Managed Servers.
#
# $HeadURL: $
...
for server_runtime in server_runtimes:
  ...
  if (cmo.getName() != aserver_name):
    ...
    if (nm_listen_address == host[0] or nm_listen_ip_address == host[1]):
      ...
      print 'SERVER ' + cmo.getName() + ' runs on a different host and will not be stopped';
  else:
    print 'SERVER ' + cmo.getName() + ' is the Admin Server and will not be stopped';
    cd('..');
