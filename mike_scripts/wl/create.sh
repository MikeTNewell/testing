...
create_basic_domain() {
    ${WEBLOGIC_HOME}/common/bin/wlst.sh -loadProperties ${SCRIPT_PATH}/../environment.properties ${SCRIPT_PATH}/create_basic_domain.py
...
pack_domain() {
    ...
    echo 'PACKING DOMAIN'
    ${WEBLOGIC_HOME}/common/bin/pack.sh -managed=true -domain=${CONFIGURATION_HOME}/domains/${DOMAIN_NAME} -template=${TEMPORARY_DIRECTORY}/templates/${DOMAIN_NAME}.jar -template_name=${DOMAIN_NAME}
}
